import { PROXY_BASE_URL, OSTHESSEN_NEWS_BASE_URL } from "../../constants";
...
export class SelectorMagicOsthessenNews extends BaseSelectorMagic {
    ...
    retrieveArticleImageLinks(doc: Document) {
        let showImageElement = Array.from(doc.querySelectorAll('img[src^="https://imgc2.osthessen-news.de/show/"]'));

        const articleRawLinkedImageElements = Array.from(doc.querySelectorAll('img[src^="https://imgc2.osthessen-news.de/images/"]'));
    ...
    retrieveArticleReadMoreLink(doc: Document) {
        ...
        let aTag = (doc.querySelector(`${nonSportSelector},${sportSelector}`) as HTMLLinkElement);
