import { OSTHESSEN_ZEITUNG_BASE_URL } from '$lib/constants';
import { sponsorHater } from '$lib/stores';
...
export class SelectorMagicOsthessenZeitung extends BaseSelectorMagic {
	...
	constructor(integrations: Integrations, requests: Requests) {
		super(integrations, requests);
	...
	retrieveArticleImageLinks(doc: Document) {
		let articleNormalImagesElements = Array.from(
			doc.querySelectorAll('img[src^="fileadmin/user_upload/"]')
		);

		// more aggresive specific query if wants to block even sponsor images for e.g. soccer games
		// if (this.isSponsorHater) {
		// 	articleNormalImagesElements = Array.from(
		// 		doc.querySelectorAll('img[src^="fileadmin/user_upload/user_upload/"]')
		// 	);
		// }

		const articleSlideshowImageElements = Array.from(
			doc.querySelectorAll('img[src^="fileadmin/fotostrecken/"]')
		);
	...
	retrieveArticleReadMoreLink(doc: Document) {
		...
		if (aTag && aTag.href) {
			const cleanedArticleSlug = aTag.href
				.replace(OSTHESSEN_ZEITUNG_BASE_URL, '')
				.replace('.html', '');
	...
	/**
	 * Osthessen Zeitung does not have ads in articles but if they would have this would be the place to check for typical ad elements and classify as ad or not.
	 */
	// eslint-disable-next-line @typescript-eslint/no-unused-vars
	isAd(doc: Document): boolean {
		return (
			doc
				.querySelector('span.news-list-category')
				?.textContent?.toLowerCase()
				.includes('anzeige') || false
		);
	}
}
