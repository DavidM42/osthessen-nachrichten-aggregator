...
export class ProcessingOsthessenNews extends BaseProcessing {
	...
	processArticle(doc: Document): ArticleType {
		...
		const manuallyExtractedTitle =
			this.selectorMagic.retrieveOverviewArticleTitle(docCopyForReadability);
		...
		const cleanedArticle = {
			...
			siteName: Integrations.OSTHESEN_NEWS,
			isAd: this.selectorMagic.isAd(doc)
		};
	...
	processHtmlElementsAsArticle(elements: Element[]): ArticleType[] {
		...
		let articles = docs.map((doc) => this.processArticle(doc));
		if (this.isSponsorHater) {
			articles = articles.filter((article) => !article.isAd);
		}
		return articles;
	}
}
